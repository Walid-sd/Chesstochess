...
function recordPuzzleIfSolved(){if(isPlay())return;const title=document.getElementById('feedbackTitle')?.textContent||'';if(!title.startsWith('Solved —'))return;const theme=document.getElementById('puzzleTheme')?.textContent||'puzzle';const body=document.getElementById('feedbackText')?.textContent||'';const signature=theme+'|'+body;if(signature===lastRecordedPuzzle)return;lastRecordedPuzzle=signature;const score=(body.match(/Score:\s*(\d+)%/i)||[])[1];store()?.recordTraining({puzzleId:theme.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,''),motifs:[theme],solved:true,wrongAttempts:parseNumber(body,'wrong attempt'),hints:parseNumber(body,'hint'),accuracy:score?Number(score):100})}
